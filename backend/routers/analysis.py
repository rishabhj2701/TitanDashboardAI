...
def _resolve_cv_analysis_context(cur) -> Dict[str, Any]:
    ...
    if has_road_stats and road_join_key_expr:
        if _norm_sql_type(road_join_key_type) == _norm_sql_type(road_stats_way_id_type):
            road_stats_join_sql = f"LEFT JOIN {road_stats_table} rs ON rs.way_id = {road_join_key_expr}"
        else:
            road_stats_join_sql = f"LEFT JOIN {road_stats_table} rs ON rs.way_id::text = ({road_join_key_expr})::text"
    ...
    road_name_expr = _preferred_road_name_expr(
        ref_expr="rs.ref" if has_road_stats else None,
        label_expr="rs.label" if has_road_stats else None,
        name_expr="rs.name" if has_road_stats else None,
        highway_expr="initcap(replace(rs.highway::text, '_', ' '))" if has_road_stats else None,
...
def analyze_crash(
    ...
    try:
                ...
                cur.execute(
                    ...
                    JOIN LATERAL (
                      SELECT
                        ...
                        {_preferred_road_name_expr(
                            ref_expr="rs.ref",
                            label_expr="rs.label",
                            name_expr="rs.name",
                            highway_expr="initcap(replace(rs.highway::text, '_', ' '))",
                        )} AS road_name
        ...
        scope = f"Crash analysis (CV +/-{int(params.get('window_minutes', payload.window_minutes))} min, {float(params.get('distance_m', payload.distance_m)):.0f}m"
        ...
        response_lines.extend([
            f"- CV points: {int(points):,}",
            f"- Vehicles: {int(vehicles):,}",
            f"- Hard braking events (<= -0.2g): {int(hb_events):,} across {int(hb_vehicles):,} vehicles",
        ])
        ...
        with postgis_store._conn() as conn, conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            ...
            try:
                ...
                hb_road_stats_join_sql = (
                    f"LEFT JOIN {road_stats_table} rs ON rs.way_id::text = {hb_way_expr}"
                ...
                hb_road_name_expr = _preferred_road_name_expr(
                    ref_expr="rs.ref" if hb_road_stats_join_sql else None,
                    label_expr="rs.label" if hb_road_stats_join_sql else None,
                    name_expr="rs.name" if hb_road_stats_join_sql else None,
                    highway_expr="initcap(replace(rs.highway::text, '_', ' '))" if hb_road_stats_join_sql else None,
        ...
        with postgis_store._conn() as conn, conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            ...
            if "road_name" not in wz_row:
                if cv_ctx["has_road_stats"]:
                    cur.execute(
                        f"""
                        SELECT {_preferred_road_name_expr(
                            ref_expr="rs.ref",
                            label_expr="rs.label",
                            name_expr="rs.name",
                            highway_expr="initcap(replace(rs.highway::text, '_', ' '))",
