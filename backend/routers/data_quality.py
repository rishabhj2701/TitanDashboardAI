...
SEGMENT_STATS_TABLE = "public.cv_route_segment_stats"
MV_ROAD_NAME_EXPR = (
    "COALESCE(NULLIF(label::text, ''), NULLIF(name::text, ''), 'Way ' || way_id::text)"
)


def _relation_exists(cur, relation: str) -> bool:
    cur.execute("SELECT to_regclass(%s) AS rel", (relation,))
    return bool((cur.fetchone() or {}).get("rel"))


def _cv_points_count(cur) -> int:
    if not _relation_exists(cur, "public.cv_points"):
        return 0
    cur.execute("SELECT COUNT(*) AS total FROM cv_points")
    return int((cur.fetchone() or {}).get("total", 0) or 0)


def _segment_stats_count(cur) -> int:
    if not _relation_exists(cur, SEGMENT_STATS_TABLE):
        return 0
    cur.execute(f"SELECT COUNT(*) AS total FROM {SEGMENT_STATS_TABLE}")
    return int((cur.fetchone() or {}).get("total", 0) or 0)


def _use_segment_stats(cur) -> bool:
    """Use binned segment aggregates when point-level CV rows are unavailable."""
    return _cv_points_count(cur) == 0 and _segment_stats_count(cur) > 0
...
def cv_data_quality(x_session_id: Optional[str] = Header(None, alias="X-Session-Id")):
    ...
    try:
        with postgis_store._conn() as conn, conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            if _use_segment_stats(cur):
                total_rows = _segment_stats_count(cur)
                fields = {}
                for col in ("route_id", "segment_start_measure", "timestamp_5min", "speed_mean_mph", "journeyid_count"):
                    cur.execute(
                        f"SELECT COUNT({col}) AS non_null, COUNT(*) - COUNT({col}) AS null_count "
                        f"FROM {SEGMENT_STATS_TABLE}"
                    )
                    row = cur.fetchone() or {}
        ...
        with postgis_store._conn() as conn, conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            if _use_segment_stats(cur):
                cur.execute(
                    f"""
                    SELECT
                      COALESCE(SUM(journeyid_count), 0) AS cv_count,
                      ROUND(AVG(speed_mean_mph)::numeric, 1) AS avg_speed,
                      ROUND(MAX(speed_max_mph)::numeric, 1) AS max_speed
                    FROM {SEGMENT_STATS_TABLE}
                    WHERE speed_mean_mph IS NOT NULL
                    """
                )
                row = cur.fetchone() or {}
                cv_count = int(row.get("cv_count") or 0)
                avg_speed = float(row.get("avg_speed") or 0)
                max_speed = float(row.get("max_speed") or 0)
            else:
                cur.execute("SELECT COUNT(*) AS total FROM cv_points")
...
def cv_speed_compliance(x_session_id: Optional[str] = Header(None, alias="X-Session-Id")):
    ...
    try:
        with postgis_store._conn() as conn, conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            if _use_segment_stats(cur):
                # Iowa segment aggregates have overspeed counts but no posted limit per row.
                cur.execute(
                    f"""
                    SELECT
                        COUNT(*) AS total,
                        0::bigint AS within_limit,
                        0::bigint AS over_limit,
                        COUNT(*) AS no_limit_data
                    FROM {SEGMENT_STATS_TABLE}
                    """
                )
            else:
    ...
    try:
        with postgis_store._conn() as conn, conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            ...
            if has_mv:
                cur.execute(
                    f"""
                    SELECT {MV_ROAD_NAME_EXPR} AS road_name,
            ...
            if _use_segment_stats(cur):
                cur.execute(
                    ...
                    SELECT hour::int AS hour,
                           ROUND(AVG(speed_mean_mph)::numeric, 1) AS avg_speed,
                           COALESCE(SUM(journeyid_count), 0) AS point_count
                    FROM {SEGMENT_STATS_TABLE}
                    WHERE hour IS NOT NULL AND speed_mean_mph IS NOT NULL
                    GROUP BY hour
                    ORDER BY hour
                    """
                )
    ...
    try:
        with postgis_store._conn() as conn, conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            if _use_segment_stats(cur):
                cur.execute(
                    f"""
                    SELECT 'Iowa (segment aggregates)' AS county,
                           COALESCE(SUM(journeyid_count), 0) AS point_count,
                           ROUND(AVG(speed_mean_mph)::numeric, 1) AS avg_speed
                    FROM {SEGMENT_STATS_TABLE}
                    """
    ...
    try:
        with postgis_store._conn() as conn, conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            if _use_segment_stats(cur):
                cur.execute(
                    f"""
                    SELECT 'Segment aggregate' AS func_class,
                           COUNT(*) AS point_count,
                           ROUND(AVG(speed_mean_mph)::numeric, 1) AS avg_speed,
                           NULL::numeric AS avg_speed_limit
                    FROM {SEGMENT_STATS_TABLE}
                    """
                )
            else:
    ...
    try:
        with postgis_store._conn() as conn, conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            if _use_segment_stats(cur):
                cur.execute(
                    f"""
                    SELECT (FLOOR(speed_mean_mph / %s) * %s)::int AS bucket_min,
                           (FLOOR(speed_mean_mph / %s) * %s + %s)::int AS bucket_max,
                           COUNT(*) AS point_count
                    FROM {SEGMENT_STATS_TABLE}
                    WHERE speed_mean_mph IS NOT NULL
                    GROUP BY FLOOR(speed_mean_mph / %s)
                    ORDER BY bucket_min
                    """,
    ...
    try:
        with postgis_store._conn() as conn, conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            if _use_segment_stats(cur):
                cur.execute(
                    f"""
                    SELECT EXTRACT(ISODOW FROM timestamp_5min)::int AS dow,
                           ROUND(AVG(speed_mean_mph)::numeric, 1) AS avg_speed,
                           COALESCE(SUM(journeyid_count), 0) AS point_count
                    FROM {SEGMENT_STATS_TABLE}
                    WHERE timestamp_5min IS NOT NULL AND speed_mean_mph IS NOT NULL
                    GROUP BY EXTRACT(ISODOW FROM timestamp_5min)::int
                    ORDER BY dow
                    """
                )
            else:
        ...
        with postgis_store._conn() as conn, conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            ...
            if _use_segment_stats(cur):
                cur.execute(
                    f"""
                    SELECT hour::int AS hour,
                           COALESCE(SUM(journeyid_nunique), 0) AS total_vehicles
                    FROM {SEGMENT_STATS_TABLE}
                ...
                cur.execute(
                    f"""
                    SELECT COALESCE(SUM(journeyid_nunique), 0) AS total_unique_vehicles
                    FROM {SEGMENT_STATS_TABLE}
                    """
                )
                summary = cur.fetchone() or {}
