...
const resolveRunDateRange = (run: CvRunSummary): { start: Date | null; end: Date | null } => {
  ...
  const fromRunId = parseRunTimestamp(run.run_id);
  if (fromRunId) return { start: fromRunId, end: fromRunId };
...
const formatProbeCount = (value?: number): string => {
  if (typeof value !== 'number' || !Number.isFinite(value) || value <= 0) return '';
  if (value >= 1_000_000) return `${(value / 1_000_000).toFixed(1)}M probes`;
  if (value >= 1_000) return `${(value / 1_000).toFixed(1)}k probes`;
  return `${value.toLocaleString()} probes`;
};
...
export const buildRunOptionLabel = (run: CvRunSummary): string => {
  ...
  const points = formatProbeCount(run.point_count);
