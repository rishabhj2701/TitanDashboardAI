...
import type { RoadAggregateFilter } from '../features/map/types';
...
type UseChatSendParams = {
  ...
  onRoadAggregateFilter?: (filter: RoadAggregateFilter) => void;
  onClearHistory?: () => void;
...
export const useChatSend = ({
  ...
  const handleSend = useCallback(async (userMessage: string) => {
    ...
    try {
      ...
      const iowaRouteIdFromText = (() => {
        const ramsId = '([A-Za-z][A-Za-z0-9]{7,18}[NESW]?)';
        const whereMatch = userMessage.match(new RegExp(`\\bwhere\\s+is\\s+${ramsId}`, 'i'));
        if (whereMatch?.[1]) return whereMatch[1].toUpperCase();
        const mapMatch = userMessage.match(new RegExp(`\\bmap\\s+${ramsId}`, 'i'));
        if (mapMatch?.[1]) return mapMatch[1].toUpperCase();
        const bareMatch = userMessage.trim().match(new RegExp(`^${ramsId}$`, 'i'));
        if (bareMatch?.[1]) return bareMatch[1].toUpperCase();
        return null;
      })();

      const normalizedRoadAggregateFilter: RoadAggregateFilter | null = roadAggregateFilter
        ? {
            ...
            road_segment_ids: Array.isArray(roadAggregateFilter.road_segment_ids)
              ? roadAggregateFilter.road_segment_ids.filter((v: any) => typeof v === 'string' && v.trim().length > 0)
              : undefined,
      ...
      const followUpRouteFilter: RoadAggregateFilter | null = iowaRouteIdFromText
        ? {
            road_segment_id: iowaRouteIdFromText,
            road_segment_ids: [iowaRouteIdFromText],
          }
        : null;

      const effectiveRoadFilter: RoadAggregateFilter | null =
        normalizedRoadAggregateFilter ?? followUpRouteFilter;

      const hasRoadAggregateFilter = Boolean(
        effectiveRoadFilter &&
          (
            effectiveRoadFilter.road_name ||
            effectiveRoadFilter.road_segment_id ||
            (effectiveRoadFilter.road_segment_ids && effectiveRoadFilter.road_segment_ids.length > 0) ||
            (effectiveRoadFilter.road_names && effectiveRoadFilter.road_names.length > 0)
          )
      );

      if (hasRoadAggregateFilter && onRoadAggregateFilter) {
        onRoadAggregateFilter(effectiveRoadFilter!);
        actionTaken = true;
      }
