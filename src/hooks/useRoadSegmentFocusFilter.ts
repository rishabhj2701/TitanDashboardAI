...
export const useRoadSegmentFocusFilter = (params: UseRoadSegmentFocusFilterParams) => {
  ...
  useEffect(() => {
    ...
    if (roadAggregateFilter) {
      ...
      const scopedSegmentIds = Array.from(
        new Set(
          [
            ...(Array.isArray(roadAggregateFilter.road_segment_ids)
              ? roadAggregateFilter.road_segment_ids
              : []),
            ...(roadAggregateFilter.road_segment_id ? [roadAggregateFilter.road_segment_id] : []),
          ]
      ...
      applyRoadSegmentFocusFilter(
        scopedSegmentIds.length ? scopedSegmentIds : null,
        scopedRoadNames.length ? scopedRoadNames : null
      );
      return;
    }
