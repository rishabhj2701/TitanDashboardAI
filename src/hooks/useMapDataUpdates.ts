...
export const useMapDataUpdates = (params: UseMapDataUpdatesParams) => {
  ...
  useEffect(() => {
    ...
    const updateMapData = () => {
      console.log('[Map] updateMapData fired', {
        mapReady,
        mapInitialized,
        mapDatasetCount: mapDataset.length,
      });
      ...
      if (dataForMap.length === 0) {
        ...
        removeDeckOverlay();
        syncLayerVisibility();
        console.log('ℹ️ No point data to render; showing road layers only.');
        return;
      }
      const now = Date.now();
      ...
      try {
        ...
        console.log(`✅ Updated map with ${geojson.features.length} points`);
        syncLayerVisibility();
