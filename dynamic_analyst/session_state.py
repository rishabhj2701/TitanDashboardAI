...
def append_execution_event(event: Dict[str, Any], session_id: Optional[str] = None):
    ...
    if client:
        ...
        client.rpush(key, json.dumps(row, separators=(",", ":"), ensure_ascii=False))
...
def _merge_maps(maps_by_type: Dict[str, Any]) -> Optional[Dict]:
    ...
    for map_type, map_data in maps_by_type.items():
        ...
        # Preserve extra map metadata when possible.
        if isinstance(map_data.get("roadAggregateFilter"), dict) and not road_aggregate_filter:
            road_aggregate_filter = map_data.get("roadAggregateFilter")
