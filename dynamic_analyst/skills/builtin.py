...
LOWEST_AVG_SPEED_ROUTES = SkillSpec(
    skill_id="lowest_avg_speed_routes",
    version="1.0.0",
    description="Rank and map the slowest routes by average CV speed (Iowa segment aggregates).",
    intent_tags=["lowest_speed", "ranking_query", "map_scope"],
    domain="traffic",
    required_inputs=[],
    input_schema={
        "type": "object",
        "properties": {
            ...
            "min_avg_speed_mph": {"type": "number", "minimum": 0, "maximum": 120},
            "dataset_id": {"type": "string"},
        },
    },
    safety_checks=[
...
def build_builtin_plan(skill_id: str, args: dict[str, Any]) -> dict[str, Any]:
    ...
    if sid == "road_corridor_map_scope":
        ...
        is_rams_route_id = bool(re.match(r"^[A-Za-z][A-Za-z0-9]{7,18}[NESW]?$", road_value))
        if is_rams_route_id:
            plan = {
                "domain": "traffic",
                "reasoning": "Built-in skill: highlight Iowa RAMS route on road layer",
                "steps": [
                    {
                        "operation": "highlight_route_ids",
                        "params": {"road_segment_id": road_value.upper()},
                    },
                ],
            }
        else:
    ...
    if sid == "lowest_avg_speed_routes":
        limit = int(payload.get("limit") or 10)
        limit = max(1, min(50, limit))
        min_avg = float(payload.get("min_avg_speed_mph") or 0.5)
        min_avg = max(0.0, min(120.0, min_avg))
        plan = {
            "domain": "traffic",
            "reasoning": "Built-in skill: slowest routes by average CV speed",
            "steps": [
                {
                    "operation": "lowest_avg_speed_routes",
                    "params": {"limit": limit, "min_avg_speed_mph": min_avg},
                }
            ],
        }
        dataset_id = str(payload.get("dataset_id") or "").strip()
