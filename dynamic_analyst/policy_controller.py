...
_RAMS_ROUTE_ID_RE = re.compile(r"\b([A-Z][A-Z0-9]{7,18}[NESW]?)\b")


def _extract_rams_route_ids(text: str, max_items: int = 4) -> list[str]:
    found: list[str] = []
    seen: set[str] = set()
    for m in _RAMS_ROUTE_ID_RE.finditer(str(text or "").upper()):
        ref = m.group(1).strip()
...
def _looks_like_lowest_avg_speed_request(q: str) -> bool:
    return (
        any(token in q for token in ("lowest", "slowest", "bottom"))
        and "speed" in q
        and any(token in q for token in ("segment", "route", "road", "plot", "map", "show", "display"))
    )
...
def _match_candidate(user_query: str) -> PolicyDecision:
    ...
    if _looks_like_lowest_avg_speed_request(q):
        limit = 10
        m = re.search(r"\b(\d{1,2})\s+(?:segment|route|road)", q)
        if m:
            limit = max(1, min(50, int(m.group(1))))
        args = {"limit": limit, "min_avg_speed_mph": 0.5}
        return PolicyDecision(
            mode="skill",
            intent_key="traffic_query",
            skill_id="lowest_avg_speed_routes",
            args=args,
            hard_rule=True,
            reason="lowest_avg_speed_routes",
            candidate_skills=(
    ...
    rams_ids = _extract_rams_route_ids(user_query, max_items=1)
    if rams_ids and any(
        token in q
        for token in ("where is", "where's", "locate", "find", "show me", "zoom", "map")
    ):
        args = {"road_name_or_ref": rams_ids[0], "map_limit": 2000}
        return PolicyDecision(
            mode="map_skill",
            intent_key="map_scope",
            skill_id="road_corridor_map_scope",
            args=args,
            hard_rule=True,
            reason="rams_route_map_scope",
            candidate_skills=(
                _candidate_skill_entry(
                    skill_id="road_corridor_map_scope",
                    args=args,
