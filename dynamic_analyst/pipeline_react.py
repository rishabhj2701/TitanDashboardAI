...
def _load_iowa_agent_context() -> str:
    """Load optional Iowa domain markdown context for the system prompt."""
    skills_dir = Path(__file__).resolve().parent / "skills"
    parts: list[str] = []
    for name in ("iowa_crash_context.md", "iowa_cv_context.md"):
...
def build_react_agent(
    ...
    *,
    ...
    iowa_context = _load_iowa_agent_context()
    system_parts = [
        ...
        "8. DOMAIN ROUTING (critical):",
        "   • Crash counts, fatalities, severity, counties, crash routes → domain=crash.",
        "   • CV speeds, hard braking, 5-minute bins, hour-of-day → domain=traffic.",
        "   • 'Top N routes by crash count' is ALWAYS domain=crash (group_by routeid), never traffic/CV.",
        "   • For Iowa CV time filters use column hour (0-23), not start_ts, unless schema lists start_ts.",
        "9. QUERYABLE FIELDS: Crash and workzone datasets have a configured set of queryable",
