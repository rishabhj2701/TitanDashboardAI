...
def _text_key_join_condition(left_expr: str, right_expr: str) -> str:
    """Compare join keys as trimmed text so numeric/text road ids can coexist."""
    return f"NULLIF(TRIM(({left_expr})::text), '') = NULLIF(TRIM(({right_expr})::text), '')"
...
def _resolve_cv_enrichment_sql(
    ...
    *,
    ...
    if roads_table and way_id_expr:
        joins.append(
            f"LEFT JOIN {roads_table} orm ON {_text_key_join_condition('orm.way_id', way_id_expr)}"
        )
    ...
    if stats_table and way_id_expr:
        joins.append(
            f"LEFT JOIN {stats_table} rs ON {_text_key_join_condition('rs.way_id', way_id_expr)}"
        )
