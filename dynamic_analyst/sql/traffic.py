...
from .traffic_aggregates import (
    ...
    _try_run_route_segment_speed_filter_operation,
    _try_run_top_hard_braking_route_segments_operation,
    _try_run_lowest_avg_speed_routes_operation,
    _try_run_highlight_route_ids_operation,
)
...
def get_traffic_domain_schema(dataset_id: Optional[str] = None) -> dict:
    ...
    try:
        with _db_conn() as conn:
            ...
            notes.append(
                "Road names use spaces not hyphens: e.g. 'I 70', 'I 44', 'US 61', 'MO 100'. "
                "Never use hyphenated forms like 'I-70'. "
                "Always filter road names with operator='ilike' and value='%I 70%' (ILIKE pattern) "
                "rather than exact equality, because partial matches are more reliable."
            )
            if stats_table and "cv_route_segment_stats" in str(stats_table):
                notes.append(
                    "Iowa CV segment bins: use hour (0-23) for time-of-day filters (e.g. 10 PM -> hour=22). "
                    "timestamp_5min and start_ts refer to the same bin timestamp. "
                    "Crash counts by route require domain=crash, not traffic."
                )
                for alias in ("route_id", "hour", "timestamp_5min", "start_ts", "speed_mean_mph", "decel_03g_sum"):
                    if alias not in groupable_columns and alias in cols_set:
                        groupable_columns.append(alias)
        ...
        with _db_conn() as conn:
            ...
            if not hb_table:
                return {
                    ...
                    "aggregate_columns": [],
                    "allowed_aggregations": ["sum", "mean", "avg", "min", "max", "count"],
                    "notes": ["Hard-braking source table is not available in this environment."],
                }
...
def run_traffic_sql_operations(dataset: str, query: Dict[str, Any]) -> str:
    ...
    try:
        ...
        lowest_avg_speed_req: Optional[dict] = parsed_steps["lowest_avg_speed_req"]
        highlight_route_ids_req: Optional[dict] = parsed_steps["highlight_route_ids_req"]
        ...
        if top_hard_braking_req is not None:
            route_hb_result = _try_run_top_hard_braking_route_segments_operation(
                top_hard_braking_req,
                dataset_id=dataset_id,
                reasoning=reasoning,
            )
            if route_hb_result is not None:
                return route_hb_result

        if highlight_route_ids_req is not None:
            highlight_result = _try_run_highlight_route_ids_operation(
                highlight_route_ids_req,
                reasoning=reasoning,
            )
            if highlight_result is not None:
                return highlight_result

        if lowest_avg_speed_req is not None:
            lowest_speed_result = _try_run_lowest_avg_speed_routes_operation(
                lowest_avg_speed_req,
                dataset_id=dataset_id,
                reasoning=reasoning,
            )
            if lowest_speed_result is not None:
                return lowest_speed_result

        route_speed_result = _try_run_route_segment_speed_filter_operation(
            dataset_id=dataset_id,
            filters=filters,
        ...
        if route_speed_result is not None:
            return route_speed_result
        ...
        with _db_conn() as conn:
            ...
            if hard_brake_only and hard_brake_table:
                ...
                base_table = _first_existing_relation(conn, _cv_relation_candidates(conn, "cv_points")) or "cv_points"
        ...
        if needs_roads and legacy_roads_table:
            from_sql += (
                f" LEFT JOIN {legacy_roads_table} r"
                f" ON NULLIF(TRIM((r.road_segment_id)::text), '')"
                f" = NULLIF(TRIM(({col_map['road_segment_id']})::text), '')"
            )
        ...
        if top_hard_braking_req is not None:
            if not hard_brake_only:
                raise ValueError("top_hard_braking_roads is only supported for hard_braking domain queries.")
        ...
        if unique_values_req is not None:
            ...
            group_cols = groupby_req.get("group_by") or []
