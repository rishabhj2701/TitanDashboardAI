...
from .traffic_aggregates import _fetch_segment_geometries_by_ids

def _crash_segment_key_sql(alias: str = "e") -> str:
    return (
        f"COALESCE(NULLIF({alias}.road_segment_id,''), NULLIF({alias}.way_id::text,''))"
    )


def _publish_crash_segment_scope_map(
    *,
    log,
    ...
    crash_col_map: dict[str, str],
    road_name_expr_map: str,
    map_req: dict,
    map_label: str,
    dataset_id: str,
) -> int:
    """
    Plot crashes on the map and highlight CV/RAMS segments (state + county network).
    """
    seg_ids = [str(v).strip() for v in (segment_ids or []) if str(v or "").strip()]
    names = [str(v).strip() for v in (road_names or []) if str(v or "").strip()]
    limit = int(map_req.get("limit", 10000))
    grid_deg = float(map_req.get("grid_deg", 0.0015))
    per_cell = int(map_req.get("per_cell", 6))

    map_where_sql = where_sql
    map_where_params = list(where_params)
    if seg_ids:
        map_where_sql = f"{where_sql} AND {_crash_segment_key_sql('e')} = ANY(%s::text[])"
        map_where_params = list(where_params) + [seg_ids[:40]]
    ...
    map_sql = f"""
        ...
        SELECT
            ...
            e.lon AS longitude,
            e.ts AS timestamp,
            {_crash_segment_key_sql('e')} AS road_segment_id,
    ...
    with _db_conn() as conn, conn.cursor(cursor_factory=RealDictCursor) as cur:
        cur.execute(
            map_sql,
            [grid_deg, grid_deg] + map_where_params + [per_cell, limit],
        )
        rows = cur.fetchall()

    lines, _road_markers = _fetch_segment_geometries_by_ids(seg_ids, limit=40)
    payload = _make_crash_map_payload(rows, label=map_label)
    payload["lines"] = lines
    payload["overlay"] = True
    payload["analysis_type"] = "crash_segments"

    road_filter: dict[str, Any] = {"min_points": 1, "limit": min(max(len(seg_ids), len(names)), 40)}
    if names:
        road_filter["road_names"] = names[:40]
        if len(names) == 1:
            road_filter["road_name"] = names[0]
    if seg_ids:
        road_filter["road_segment_ids"] = seg_ids[:40]
        road_filter["road_segment_id"] = seg_ids[0]
    payload["roadAggregateFilter"] = road_filter

    save_map_for_session(payload, map_type="crash")
    log.info(
        json.dumps(
            {
                "event": "crash_segment_scope_map",
...
def _crash_roads_join_clause(
    conn,
    roads_table: Optional[str],
    event_alias: str = "e",
) -> str:
    if not roads_table:
        return ""
    cols = _table_column_names(conn, roads_table)
    if "road_segment_id" in cols:
        return (
            f"LEFT JOIN {roads_table} r "
            f"ON r.road_segment_id = {event_alias}.road_segment_id"
        )
    if "way_id" in cols:
        return (
            f"LEFT JOIN {roads_table} r ON r.way_id::text = COALESCE("
            f"NULLIF({event_alias}.road_segment_id,''), "
            f"NULLIF({event_alias}.props->>'way_id',''))"
        )
    return ""


def _crash_enriched_table(conn) -> Optional[str]:
    return _first_existing_relation(
        conn,
        ["app_data.crash_road_enriched", "crash_road_enriched"],
    )


def _apply_crash_enriched_col_map(crash_col_map: dict[str, str]) -> None:
    crash_col_map["road_name"] = "e.road_name"
    crash_col_map["road"] = "e.road_name"
    crash_col_map["name"] = "e.road_name"
    crash_col_map["road_segment_id"] = _crash_segment_key_sql("e")
    crash_col_map["way_id"] = "e.way_id::text"
    crash_col_map["routeid"] = "COALESCE(e.routeid_raw, e.route_base_ref)"
    crash_col_map["route_id"] = crash_col_map["routeid"]
    crash_col_map["route_base_ref"] = "e.route_base_ref"
    crash_col_map["route_prefix"] = "e.route_prefix"
    crash_col_map["speed_limit_mph"] = "e.effective_speed_limit_mph"
    crash_col_map["effective_speed_limit_mph"] = "e.effective_speed_limit_mph"
    crash_col_map["segment_speed_limit_mph"] = "e.speed_limit_mph"
    crash_col_map["avg_speed_mph"] = "e.avg_speed_mph"
    crash_col_map["segment_avg_speed_mph"] = "e.segment_avg_speed_mph"
    if "e.severity" not in str(crash_col_map.get("severity", "")):
        crash_col_map["severity"] = "e.severity"
    crash_col_map["accident_date"] = (
        "COALESCE("
        "NULLIF(e.accident_date,''), "
        f"TO_CHAR((e.ts AT TIME ZONE '{CRASH_TIMEZONE}')::date, 'YYYY-MM-DD')"
        ")"
    )
    crash_col_map["accident_time"] = (
        "COALESCE("
        "NULLIF(e.accident_time,''), "
        f"TO_CHAR((e.ts AT TIME ZONE '{CRASH_TIMEZONE}')::time, 'HH24:MI:SS')"
        ")"
    )
    crash_col_map["local_time"] = f"(e.ts AT TIME ZONE '{CRASH_TIMEZONE}')::time"
    crash_col_map["local_hour"] = (
        f"EXTRACT(HOUR FROM (e.ts AT TIME ZONE '{CRASH_TIMEZONE}'))"
    )


def _crash_road_name_parts(roads_table: Optional[str], mapped_road_expr: Optional[str] = None) -> list[str]:
    parts: list[str] = []
    if mapped_road_expr:
        parts.append(mapped_road_expr)
    if roads_table and "rams" in str(roads_table).lower():
        parts.extend(["r.label", "r.name", "r.ref"])
    elif roads_table:
        parts.append("r.name")
    parts.extend(
        [
...
def get_crash_domain_schema(dataset_id: Optional[str] = None) -> dict:
    ...
    groupable_columns = [
        "road_segment_id", "way_id", "road_name", "road", "routeid", "route_id",
        "route_base_ref", "route_prefix", "measure", "city", "city_name",
        "county", "modot_county_nm", "modot_district_abbr", "modot_district_no",
        "severity", "accident_date", "event_date", "local_hour", "accident_type", "two_veh_analysis",
    ...
    return {
        ...
        "notes": [
            ...
            "Location columns: road_name, routeid (e.g. S-0035, C-130), measure, city, county.",
            "Iowa DOT routeid values look like S001910035N (no hyphens); filter with ilike (e.g. %0035%).",
...
def run_crash_sql_operations(dataset: str, query: Dict[str, Any]) -> str:
    ...
    try:
        ...
        filters: list[dict] = []
        filter_mode = "and"
        sparse_window_note: Optional[str] = None
        datetime_placeholder_note: Optional[str] = None
        ...
        for step in steps:
            ...
            if op == "filter":
                note = params.get("sparse_window_note")
                if note:
                    sparse_window_note = str(note)
                ph_note = params.get("datetime_placeholder_note")
                if ph_note:
                    datetime_placeholder_note = str(ph_note)
        ...
        use_crash_enriched = False
        with _db_conn() as conn:
            enriched_table = _crash_enriched_table(conn)
            roads_table = _first_existing_relation(
                conn,
                [
                    "public.rams_roads_match",
                    "rams_roads_match",
                    "public.roads",
                    "roads",
                ],
            )
            roads_join_sql = _crash_roads_join_clause(conn, roads_table)
            if enriched_table:
                use_crash_enriched = True
                _apply_crash_enriched_col_map(crash_col_map)
        if not roads_table:
            road_expr_no_join = (
                "COALESCE("
                "NULLIF(e.props->>'road',''), "
        ...
        if not isinstance(enabled_query_names, list):
            enabled_query_names = []
        if use_crash_enriched:
            for extra_col in (
                "speed_limit_mph",
                "effective_speed_limit_mph",
                "segment_speed_limit_mph",
                "avg_speed_mph",
                "segment_avg_speed_mph",
                "route_base_ref",
                "route_prefix",
                "road_segment_id",
                "way_id",
            ):
                if extra_col not in enabled_query_names:
                    enabled_query_names.append(extra_col)
                queryable_alias_map.setdefault(extra_col, extra_col)
        ...
        segment_speed_join_sql = ""
        cv_stats_table: Optional[str] = None
        if not use_crash_enriched:
            with _db_conn() as conn_cv:
                cv_stats_table = _first_existing_relation(
                    conn_cv,
                    ["public.cv_road_stats_mv", "cv_road_stats_mv"],
                )

        enriched_query_cols = frozenset(
            {
                "speed_limit_mph",
                "effective_speed_limit_mph",
                "segment_speed_limit_mph",
        ...
        def _filters_need_segment_speed_limit(conditions: list[dict]) -> bool:
            for condition in _iter_leaf_conditions(conditions):
                col = str(condition.get("column") or "").strip().lower()
                if col in {
                    "speed_limit_mph",
                    "effective_speed_limit_mph",
                    "segment_speed_limit_mph",
        ...
        def _filters_need_segment_avg_speed(conditions: list[dict]) -> bool:
            for condition in _iter_leaf_conditions(conditions):
                col = str(condition.get("column") or "").strip().lower()
                if col in {"avg_speed_mph", "segment_avg_speed_mph"}:
                    return True
            return False

        if not use_crash_enriched and cv_stats_table and _filters_need_segment_speed_limit(filters):
            crash_col_map["speed_limit_mph"] = "seg.speed_limit_mph"
            crash_col_map["effective_speed_limit_mph"] = "seg.speed_limit_mph"
            for extra_col in enriched_query_cols:
                if extra_col not in enabled_query_names:
                    enabled_query_names.append(extra_col)
                queryable_alias_map.setdefault(extra_col, extra_col)

        if not use_crash_enriched and cv_stats_table and _filters_need_segment_avg_speed(filters):
            crash_col_map["avg_speed_mph"] = "seg.avg_speed_mph"
            crash_col_map["segment_avg_speed_mph"] = "seg.avg_speed_mph"
            for extra_col in enriched_query_cols:
                if extra_col not in enabled_query_names:
                    enabled_query_names.append(extra_col)
                queryable_alias_map.setdefault(extra_col, extra_col)
        ...
        def _require_queryable(col_name: Any, *, allow_count: bool = False) -> str:
            ...
            if allow_count and _normalize_metric_key(text) == "count":
                return "count"
            if text in enriched_query_cols and text in crash_col_map:
                return text
        ...
        def _get_categorical_candidates(col: str) -> list[str]:
            ...
            if "r." in expr and roads_join_sql:
                from_sql_candidates += f" {roads_join_sql}"
            sql = f"""
        ...
        def _align_categorical_filters(conditions: list[dict]) -> list[dict]:
            ...
            for cond in conditions or []:
                ...
                if col in {"accident_date", "event_date"} and isinstance(cond.get("value"), str):
                    if re.fullmatch(r"20\d{2}-\d{2}-\d{2}", cond.get("value").strip()):
                        rewritten.append(cond)
                        continue
        ...
        needs_segment_speed = _filters_need_segment_speed_limit(filters)
        needs_segment_avg_speed = _filters_need_segment_avg_speed(filters)
        if use_crash_enriched:
            from_sql = f"{enriched_table} e"
            road_name_expr = "e.road_name"
        else:
            from_sql = f"{APP_EVENTS} e"
            if (needs_segment_speed or needs_segment_avg_speed) and cv_stats_table:
                segment_speed_join_sql = (
                    f"LEFT JOIN {cv_stats_table} seg "
                    f"ON seg.way_id::text = {_crash_segment_key_sql('e')}"
                )
                from_sql += f" {segment_speed_join_sql}"
                if needs_segment_speed:
                    crash_col_map["speed_limit_mph"] = "seg.speed_limit_mph"
                    crash_col_map["effective_speed_limit_mph"] = "seg.speed_limit_mph"
                if needs_segment_avg_speed:
                    crash_col_map["avg_speed_mph"] = "seg.avg_speed_mph"
                    crash_col_map["segment_avg_speed_mph"] = "seg.avg_speed_mph"
                for extra_col in enriched_query_cols:
                    if extra_col not in enabled_query_names:
                        enabled_query_names.append(extra_col)
                    queryable_alias_map.setdefault(extra_col, extra_col)
            if join_roads and roads_join_sql:
                from_sql += f" {roads_join_sql}"
            road_name_expr = "COALESCE(" + ", ".join(_crash_road_name_parts(roads_table, mapped_road_name_expr)) + ")"

        where_sql = " AND ".join(where_parts)
        filter_where_params = list(where_params)

        map_rows_plotted: Optional[int] = None
        defer_segment_map = False
        if map_req is not None and groupby_req is not None:
            group_cols_raw = _groupby_columns_from_params(groupby_req)
            defer_segment_map = any(
                str(c).strip().lower() in ("road_segment_id", "way_id", "segment_id")
                for c in group_cols_raw
            )

        # 1) MAP SIDE-EFFECT (skip when segment-ranked table will drive scoped map)
        if map_req is not None and not defer_segment_map:
            limit = int(map_req.get("limit", 10000))
            grid_deg = float(map_req.get("grid_deg", 0.0015))
            per_cell = int(map_req.get("per_cell", 5))

            if use_crash_enriched:
                from_sql_map = f"{enriched_table} e"
                road_name_expr_map = "e.road_name"
            else:
                from_sql_map = f"{APP_EVENTS} e"
                if segment_speed_join_sql:
                    from_sql_map += f" {segment_speed_join_sql}"
                if roads_join_sql:
                    from_sql_map += f" {roads_join_sql}"
                road_name_expr_map = "COALESCE(" + ", ".join(_crash_road_name_parts(roads_table, mapped_road_name_expr)) + ")"
            ...
            with _db_conn() as conn, conn.cursor(cursor_factory=RealDictCursor) as cur:
                cur.execute(map_sql, [grid_deg, grid_deg] + filter_where_params + [per_cell, limit])
                rows = cur.fetchall()
            ...
            for cond in filters:
                if not isinstance(cond, dict):
                    continue
                col = str(cond.get("column") or "").lower()
                val = str(cond.get("value") or "").strip()
                if col in {"road_name", "road", "road_ref"} and val and "%" in val:
                    corridor = val.strip("%").replace("%", " ").strip()
                    if corridor:
                        map_label = f"Crashes — {corridor}"
                        break
            save_map_for_session(_make_crash_map_payload(rows, label=map_label), map_type="crash")

            if needs_segment_speed:
                seg_sql = f"""
                    SELECT DISTINCT {_crash_segment_key_sql('e')} AS seg_id
                    FROM {from_sql_map}
                    WHERE {where_sql}
                      AND {_crash_segment_key_sql('e')} IS NOT NULL
                    LIMIT 40
                """
                with _db_conn() as conn, conn.cursor() as cur:
                    cur.execute(seg_sql, filter_where_params)
                    speed_seg_ids = [
                        str(r[0]).strip()
                ...
                if speed_seg_ids:
                    map_rows_plotted = _publish_crash_segment_scope_map(
                        log=log,
                        segment_ids=speed_seg_ids,
                        road_names=[],
                        from_sql_map=from_sql_map,
                        where_sql=where_sql,
                        where_params=filter_where_params,
                        crash_col_map=crash_col_map,
                        road_name_expr_map=road_name_expr_map,
                        map_req=map_req,
                        map_label=map_label,
                        dataset_id=dataset_id,
                    )
        ...
        if unique_values_req is not None:
            ...
            raw_group_cols = _groupby_columns_from_params(groupby_req)
        ...
        if map_req is not None and defer_segment_map:
            seg_col = next(
                (c for c in df.columns if str(c).strip().lower() in ("road_segment_id", "way_id", "segment_id")),
                None,
            )
            name_col = next(
                (c for c in df.columns if str(c).strip().lower() in ("road_name", "road", "name", "label")),
                None,
            )
            segment_ids: list[str] = []
            road_names: list[str] = []
            if seg_col:
                for raw in df[seg_col].tolist():
                    if raw is None:
                        continue
                    seg = str(raw).strip()
            ...
            if segment_ids or road_names or defer_segment_map:
                map_label = "Crash segments"
                for cond in filters:
                    if not isinstance(cond, dict):
                        continue
                    col = str(cond.get("column") or "").lower()
                    val = str(cond.get("value") or "").strip()
                    if col in {"road_name", "road", "road_ref"} and val:
                        corridor = val.strip("%").replace("%", " ").strip()
                        if corridor:
                            map_label = f"Crashes — {corridor}"
                            break
                    if col == "road_name" and cond.get("operator") == "contains":
                        map_label = f"Crashes — {cond.get('value')}"
                        break
                if use_crash_enriched:
                    scoped_from_sql_map = f"{enriched_table} e"
                    scoped_road_expr = "e.road_name"
                else:
                    scoped_from_sql_map = f"{APP_EVENTS} e"
                    if segment_speed_join_sql:
                        scoped_from_sql_map += f" {segment_speed_join_sql}"
                    if roads_join_sql:
                        scoped_from_sql_map += f" {roads_join_sql}"
                    scoped_road_expr = "COALESCE(" + ", ".join(_crash_road_name_parts(roads_table, mapped_road_name_expr)) + ")"
                map_rows_plotted = _publish_crash_segment_scope_map(
                    log=log,
                    segment_ids=segment_ids,
                    road_names=road_names,
                    from_sql_map=scoped_from_sql_map,
                    where_sql=where_sql,
                    where_params=filter_where_params,
                    crash_col_map=crash_col_map,
                    road_name_expr_map=scoped_road_expr,
                    map_req=map_req,
                    map_label=map_label,
                    dataset_id=dataset_id,
                )
        ...
        if sparse_window_note:
            response_parts.append(sparse_window_note)
        if datetime_placeholder_note:
            response_parts.append(datetime_placeholder_note)
        ...
        elif aggregate_req is not None and map_req is not None and not df.empty:
            count_col = next(
                (c for c in df.columns if str(c).strip().lower() in ("count", "crash_count")),
                None,
            )
            if count_col is not None:
                try:
                    total_matches = int(df.iloc[0][count_col])
                except Exception:
                    total_matches = df.iloc[0][count_col]
                response_parts.append(f"Total crashes matching filters: {total_matches:,}")
                if map_rows_plotted is not None:
                    response_parts.append(
                        f"Map shows up to {int(map_rows_plotted):,} points (display sample; not the full count)."
                    )
            else:
                response_parts.append(_df_to_markdown_safe(df))
                if map_rows_plotted is not None:
                    response_parts.append(f"Map points plotted: {int(map_rows_plotted):,}")
        elif is_map_only_count and ("count" in df.columns):
            try:
                count_value = int(df.iloc[0]["count"])
            except Exception:
                count_value = df.iloc[0]["count"]
            response_parts.append(f"Total crashes matching filters: {count_value:,}")
            if map_rows_plotted is not None and int(map_rows_plotted) < int(count_value):
                response_parts.append(
                    f"Map shows up to {int(map_rows_plotted):,} points (display sample; not the full count)."
                )
