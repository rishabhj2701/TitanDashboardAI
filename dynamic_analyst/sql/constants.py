...
_CRASH_SQL_COL = {
    ...
    "county": (
        "COALESCE("
        "NULLIF(e.props->>'county',''), "
        "NULLIF(e.props->>'COUNTY',''), "
        "NULLIF(e.props->>'modot_county_nm','')"
        ")"
    ),
    "modot_county_nm": "NULLIF(e.props->>'modot_county_nm','')",
    ...
    # ── Iowa crash CSV columns ────────────────────────────────────────────────
    # CRASH_KEY — unique crash identifier
    "crash_key": (
        "COALESCE("
        "NULLIF(e.props->>'CRASH_KEY',''), "
        "NULLIF(e.props->>'crash_key','')"
        ")"
    ),
    "CRASH_KEY": (
    ...
    # CSEVERITY — Iowa KABCO severity code: 1=Fatal,2=Major,3=Minor,4=PDO,5=Unknown
    "cseverity": (
        "COALESCE("
        "NULLIF(e.props->>'CSEVERITY',''), "
        "NULLIF(e.props->>'cseverity',''), "
        "NULLIF(e.props->>'severity','')"
        ")"
    ),
    "CSEVERITY": (
        "COALESCE("
    ...
    # ROUTEID — Iowa RAMS route identifier
    "routeid": (
        "COALESCE("
        "NULLIF(e.road_segment_id,''), "
        "NULLIF(e.props->>'ROUTEID',''), "
        "NULLIF(e.props->>'routeid','')"
        ")"
    ),
    "ROUTEID": (
    ...
    # MEASURE — milepost along the route
    "measure": (
        "COALESCE("
        "NULLIF(e.props->>'MEASURE',''), "
        "NULLIF(e.props->>'measure','')"
        ")::float8"
    ),
    "MEASURE": (
    ...
    # Temporal breakdowns
    "crashmonth": "NULLIF(e.props->>'CRASHMONTH','')::int",
    "CRASHMONTH": "NULLIF(e.props->>'CRASHMONTH','')::int",
    "crash_month": "NULLIF(e.props->>'CRASHMONTH','')::int",
    "month": (
        "COALESCE("
        "NULLIF(e.props->>'CRASHMONTH','')::int, "
        f"EXTRACT(MONTH FROM (e.ts AT TIME ZONE '{CRASH_TIMEZONE}'))::int"
        ")"
    ),
    "crash_day": "NULLIF(e.props->>'CRASH_DAY','')::int",
    "CRASH_DAY": "NULLIF(e.props->>'CRASH_DAY','')::int",
    "day_of_week": "NULLIF(e.props->>'CRASH_DAY','')::int",
    "crash_year": "NULLIF(e.props->>'CRASH_YEAR','')::int",
    "CRASH_YEAR": "NULLIF(e.props->>'CRASH_YEAR','')::int",
    "year": (
        "COALESCE("
        "NULLIF(e.props->>'CRASH_YEAR','')::int, "
        f"EXTRACT(YEAR FROM (e.ts AT TIME ZONE '{CRASH_TIMEZONE}'))::int"
        ")"
    ),
    # TIMESTR — time as HH:MM string
    "timestr": "NULLIF(e.props->>'TIMESTR','')",
    "TIMESTR": "NULLIF(e.props->>'TIMESTR','')",
    "crash_time": "NULLIF(e.props->>'TIMESTR','')",
}
