...
def _parse_traffic_sql_steps(steps: list[dict]) -> dict[str, Any]:
    state: dict[str, Any] = {
        ...
        "lowest_avg_speed_req": None,
        "highlight_route_ids_req": None,
    }
    ...
    for step in steps:
        ...
        elif op == "lowest_avg_speed_routes":
            state["lowest_avg_speed_req"] = params
        elif op == "highlight_route_ids":
            state["highlight_route_ids_req"] = params
        else:
            raise ValueError(
                ...
                "(legacy: top_speed_roads, top_hard_braking_roads, lowest_avg_speed_routes)."
            )
    return state
